...
flags.DEFINE_integer('num_gpu', 0,
                     'Number of GPUs to use.')

model_dir = './model'
image_size = (1280, 720)
data_dir = '/home/gym/video/img/283/'

folder_list = file_list(data_dir)
list_file = data_dir + 'test.txt'
list_f = open(list_file,'r')
lines = list_f.readlines()
images = list()
print('[Read images]')
for line in lines:
    img = np.array(Image.open(data_dir+line[:-1]).resize(image_size, Image.ANTIALIAS)).astype(np.float32)
    images.append(img)
